  ...
  "Get Registration Action" - {
    ...
    "and a registration can be retrieved from the backend" - {
      ...
      "must save the registration to the sessionRepository and return Right" in {
        ...
        when(connector.getRegistration(any())(any())) thenReturn registrationWrapper.toFuture

        val action = new Harness(connector)

        val result = action.callRefine(IdentifierRequest(request, "id", Enrolments(Set.empty), Vrn("123456789"), "IN9001234567")).futureValue

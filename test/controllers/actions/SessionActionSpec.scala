...
class SessionActionSpec extends SpecBase {
  ...
  class Harness(action: SessionIdentifierAction, actionBuilder: DefaultActionBuilder) {
    def onPageLoad(): Action[AnyContent] = (actionBuilder andThen action) { _ => Results.Ok }
  }

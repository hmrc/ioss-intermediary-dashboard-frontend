...
class ClientsOverdueReturnsListControllerSpec extends SpecBase with BeforeAndAfterEach {
  ...
  private val incompleteReturns: Seq[Return] = Gen.listOfN(3, arbitraryReturn.arbitrary).sample.value.map(_.copy(submissionStatus = Overdue))
  ...
  private val updatedCurrentReturns: Seq[CurrentReturns] = currentReturns.zipWithIndex.map { (currentReturn, index) =>
    currentReturn.copy(iossNumber = indexedClientIossNumbers(index)._1)
      .copy(incompleteReturns = incompleteReturns)
  }

  protected lazy val clientsOverdueReturnsRoute: String = routes.ClientsOverdueReturnsListController.onPageLoad().url

  override def beforeEach(): Unit = {
    Mockito.reset(mockCurrentReturnsService)
  }

  "ClientsOverdueReturnsListController" - {

    "must return OK and the correct view for a GET when there are client Overdue returns present" in {
      ...
      running(application) {
        ...
        val request = FakeRequest(GET, clientsOverdueReturnsRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[ClientsOverdueReturnsListView]
        ...
        val link: String = routes.ClientsOutstandingReturnsListController.onPageLoad(waypoints).url

        status(result) `mustBe` OK
        contentAsString(result) `mustBe` view(viewModel, link, currentReturnsNonEmpty = true)(request, messages(application)).toString
    ...
    "must return OK and the correct view for a GET when there are no client Overdue returns present" in {
      ...
      running(application) {
        ...
        status(result) `mustBe` OK
        contentAsString(result) `mustBe` view(viewModel, link, currentReturnsNonEmpty = false)(request, messages(application)).toString
        verify(mockCurrentReturnsService, times(1)).getCurrentReturns(any())(any())
      }
    }

  ...
  "PreviousRegistration" - {
    ...
    "must serialise/deserialise to and from PreviousRegistration" in {
      ...
      val expectedJson = Json.obj(
        "intermediaryNumber" -> previousRegistration.intermediaryNumber,
    ...
    "must populate Radio Items correctly" in {
      ...
      running(application) {
        ...
        val expectedResult = Seq(
          RadioItem(
            content = HtmlContent("December 2023 to January 2024"),
            id = Some(s"value_0"),
            value = Some("IM900123456789"),
            hint = Some(Hint(content = HtmlContent("IOSS intermediary number: IM900123456789")))
          ),
          RadioItem(
            content = HtmlContent("February 2024 to March 2024"),
            id = Some(s"value_1"),
            value = Some("IM900987654321"),
            hint = Some(Hint(content = HtmlContent("IOSS intermediary number: IM900987654321")))
          ),
          RadioItem(
            content = HtmlContent("March 2024 to April 2024"),
            id = Some(s"value_2"),
            value = Some("IM900123456321"),
            hint = Some(Hint(content = HtmlContent("IOSS intermediary number: IM900123456321")))
          )
        )

...
class CheckBouncedEmailFilterImpl()(implicit val executionContext: ExecutionContext)
  ...
  override protected def filter[A](request: RegistrationRequest[A]): Future[Option[Result]] = {
    if (request.registrationWrapper.etmpDisplayRegistration.schemeDetails.unusableStatus) {
      Some(Redirect(controllers.routes.InterceptUnusableEmailController.onPageLoad().url)).toFuture
    } else {
      None.toFuture
    }
  }
...
class CheckBouncedEmailFilterProvider @Inject()()(implicit ec: ExecutionContext) {
  ...
  def apply(): CheckBouncedEmailFilterImpl = new CheckBouncedEmailFilterImpl()
}
